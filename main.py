...
pygame.mixer.music.load('img/audio_music2.mp3')
pygame.mixer.music.set_volume(0.3)
pygame.mixer.music.play(-1, 0.0, 5000)
shot_fx = pygame.mixer.Sound('img/audio_shot.wav')
...
class Castle():
    ...
    def draw(self):
        #Check which image to use based on halth
        if self.health <= 250:
...
while run:
    ...
    if start_game == False:
        ...
        if game_over == False:
            ...
            if level_difficulty >= target_difficulty:
                #Check how many are still alives
                enemies_alive = 0
                for e in enemy_group:
                    if e.alive == True:
                        enemies_alive += 1
